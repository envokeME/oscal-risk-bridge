...
The project runs locally with sample OSCAL-formatted assessment results and produces CSV, JSON, Markdown, and BattleRisk-themed HTML risk register outputs. Each risk scenario includes NIST CSF 2.0 function/category/outcome alignment. The HTML report also includes an AI analysis placeholder with embedded structured risk data for a future review workflow. I included an optional AWS pattern using S3 and Lambda, but the core workflow does not require cloud infrastructure.
